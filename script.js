...
function filterData(e) {
    ...
    const filteredArr = dataArray.filter(el => el.name.first.toLowerCase().includes(searchedString) ||
                                               ...
                                               `${el.name.last + el.name.first}`.toLocaleLowerCase().replace(/\s/g, "").includes(searchedString) ||
                                               `${el.name.first + el.name.last}`.toLocaleLowerCase().replace(/\s/g, "").includes(searchedString)
                                               )
